...
public class ModalProdutoPage {
	...
	private By botaoPrceedToCheckout = By.cssSelector("div.cart-content-btn a.btn-primary");
	...
	public String obterMensagemProdutoAdicionado() {
		...
		FluentWait wait = new FluentWait(driver).withTimeout(Duration.ofSeconds(5)).pollingEvery(Duration.ofSeconds(1))
				.ignoring(NoSuchElementException.class);// aguarda 5 segundo para o elemento aparecer,
		// e a cada 1 segundo verifica se o elemento apareceu
		// E ignora a mensagem de exceção exibida no selenium enquanto aguarda o tempo.

		wait.until(ExpectedConditions.visibilityOfElementLocated(mensagemProdutoAdicionado));// espera que o
																								// elemento(mensagem)
																								// esteja visivel
	...
	public CarrinhoPage clicarBotaoProceedToCheckout() {
		...
		driver.findElement(botaoPrceedToCheckout).click();
